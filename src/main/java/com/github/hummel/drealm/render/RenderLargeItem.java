package com.github.hummel.drealm.render;
...
public class RenderLargeItem extends LOTRRenderLargeItem {
	...
	private RenderLargeItem(Item item, String dir, float f) {
		super(item, dir, f);
		theItem = item;
		folderName = dir;
		largeIconScale = f;
	...
	public static RenderLargeItem getRendererIfLarge(Item item) {
		for (Map.Entry<String, Float> folder : SIZE_FOLDERS.entrySet()) {
			...
			try {
				...
				return new RenderLargeItem(item, folder.getKey(), iconScale);
